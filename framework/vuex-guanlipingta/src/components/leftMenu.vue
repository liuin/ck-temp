<template>
  <el-row class="tac" v-loading="_.isEmpty(menuData)">
      ...
      @open="handleOpen" 
      @close="handleClose" :default-active="$store.state.leftMenu.defaultActive"  >
      <el-submenu  v-for="(item, index) in menuData" :key="index" :index="'menu'+index">
        ...
        <el-menu-item  v-for="(itemSub, indexSub) in item.sub" :index="'/' + itemSub.action" :key="indexSub"><i v-if="itemSub.icon" :class="itemSub.icon"></i> {{itemSub.title}}</el-menu-item>        
      </el-submenu>      
    </el-menu>
  </el-col>
 </el-row>
