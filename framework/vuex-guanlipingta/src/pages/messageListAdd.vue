    ...
    <div class="box">
      <el-form label-position="right" label-width="120px">
        <el-form-item label="标题">
          <el-input v-model="templateCreate.title"></el-input>
        </el-form-item>

        <el-form-item label="分类">
          <el-select v-model="templateCreate.category_id">
        ...
        <el-row>
          <el-col :span="10">
            ...
            <el-form-item label="推送平台" v-if="tagsItemValueType == '2'">
              <el-select v-model="templateCreate.platform_id" filterable>
        ...
        <el-form-item label="正文">
          <!-- <el-input type="textarea" v-model="templateCreate.content" :rows="4"></el-input> -->
          <editHTML :content.sync="templateCreate.content"></editHTML>
          <!-- <el-input type="textarea" v-model="templateCreate.content" :rows="4"></el-input> -->
        </el-form-item>
...
export default {
  ...
  watch: {
    activeTime(newVal, oldVal) {
      this.templateCreate.activity_start_time = newVal[0].getTime() / 1000;
      this.templateCreate.activity_end_time = newVal[1].getTime() / 1000;
    },
    pushTime(newVal) {
      this.templateCreate.push_time = newVal.getTime() / 1000;
    },
    tagsItemValue(newVal) {
      if (newVal != 0) {
        var id = newVal.split("-");
        this.templateCreate.push_tags_id = id[1];
        this.templateCreate.push_tags_type = parseInt(id[0]);
        this.tagsItemValueType = id[0];
      } else {
        this.templateCreate.push_tags_id = 0;
        this.tagsItemValueType = 0;
        this.templateCreate.push_tags_type = newVal;
      }
    }
  },
  computed: {
  ...
  methods: {
    reset() {
      this.templateCreate.category_id = "";
      this.templateCreate.title = "";
      this.templateCreate.content = "";
    ...
    save(type) {
      if (type == "send") {
        this.$confirm(
          ...
          this.templateCreate.state = 2;
          if (this.uploadImg.fileList.length > 0) {
            this.$store.dispatch("uploadImg/upImg").then(data => {
              this.templateCreate.cover_image = data.path;
              this.$store.dispatch("message/templateCreate").then(data => {
                this.$alert("添加成功", "", {
                  confirmButtonText: "确定",
                  type: "success",
                  callback: action => {
    ...
    regionCahge(arr, currentLabels) {
      this.templateCreate.province = arr[0];
      this.templateCreate.city = arr[1];
    },
