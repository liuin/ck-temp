      ...
      <el-table border width="100%" :data="activityListRes" v-loading="loading">
        ...
        <el-table-column prop="" label="操作" width="250px">
          <template slot-scope="scope">
            ...
            <el-button size="mini" @click="$router.push({path: '/activity/rechargeDetail', 'query': {'id': scope.row.id}})">详情</el-button>
            <el-button size="mini" @click="$router.push({path: '/activity/rechargeEdit', 'query': {'id': scope.row.id}})">编辑</el-button>
          </template>
        </el-table-column>
      </el-table>
