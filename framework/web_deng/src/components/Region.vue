<template>
    <div class="region">
    	 <el-cascader :options="options" change-on-select ref="opt" :clearable="clearable" :separator="separator" v-model="selectedOptions" @change="handleChange" />
    </div>
</template>
<script>
        ...
        created() {
        	...
        	for(let p in RegionJson['1']){
        		if(that.limitProvince && that.limitProvince.length>0){
        			if(!that.inArray(p,that.limitProvince) && !that.inArray(RegionJson['1'][p],that.limitProvince)) continue;
        		}
        		var obj={};
        		obj.value=p;
        		...
        		if(RegionJson[p] && that.limitLen>1){
        			for(let c in RegionJson[p]){
        				if(that.limitCity && that.limitCity.length>0){
		        			if(!that.inArray(c,that.limitCity) && !that.inArray(RegionJson[p][c],that.limitCity)) continue;
		        		}
        				var obj2={};
        				obj2.value=c;
        				...
        				if(RegionJson[c] && that.limitLen>2){
        					for(let s in RegionJson[c]){
        						if(that.limitStreet && that.limitStreet.length>0){
				        			if(!that.inArray(s,that.limitStreet) && !that.inArray(RegionJson[c][s],that.limitStreet)) continue;
				        		}
        						var obj3={};
		        				obj3.value=s;
