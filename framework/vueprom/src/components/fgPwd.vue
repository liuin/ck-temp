<template>
  <div class="loginbox regbox">
    ...
    <div class="vd-box df-left-right">
      <div>
        <van-field v-model="login.imgCode" placeholder="请输入图形验证" @blur="$verify.check('imgCode')" v-verify.imgCode="login.imgCode"
          icon="clear" v-verify="login.imgCode" @click-icon="login.imgCode = ''" />
    ...
    <van-field v-model="login.password" @blur="$verify.check('password')" type="password" placeholder="输入新密码" icon="clear" v-verify.password="login.password"
      @click-icon="login.password = ''" />
    <label class="van-field__error-message" v-remind="login.password"></label>

    <van-field v-model="rpPassword" @blur="$verify.check('password')" type="password" placeholder="确认新密码" icon="clear" v-verify.password="rpPassword"
    ...
    <van-button size="large" class="btn1" @click="submit()">修改密码</van-button>
...
export default {
  ...
  methods: {
    getImg() {
      if (this.$store.state.ajaxLoad == false) {
        this.$api.ajax({
          type: "post",
